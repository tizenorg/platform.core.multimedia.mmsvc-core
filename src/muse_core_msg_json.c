		...
		muse_core_msg_parse_err_e *err, muse_core_type_e m_type)
{
	...
	if (g_muse_core_msg_json->jso) {
		LOGD("[parse len: %d] %s", g_muse_core_msg_json->parsed_len, json_object_to_json_string(g_muse_core_msg_json->jso));
		if (parse_len)
...
void muse_core_msg_json_object_free(void)
{
	...
	if (g_muse_core_msg_json) {
		LOGD("g_muse_core_msg_json free");
