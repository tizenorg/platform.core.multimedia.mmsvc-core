		...
		if (len <= 0) {
			...
			while (module->msg_offset < len) {
				if (muse_core_msg_json_deserialize(MUSE_API, module->recvMsg + module->msg_offset, &parse_len, &cmd, &err, MUSE_TYPE_INT)) {
					...
					LOGE("[Parsing status : %d] %s", err, module->recvMsg);
					break;
				}
...
static void _muse_core_ipc_msg_concatenate(muse_client_h client, char *msg, int msg_len)
{
	...
	if (client->cache_len > 0) {
		char *strconcat = g_strconcat(client->cache, msg, NULL);
		g_return_val_if_fail(strconcat != NULL, RECV_ERR);
		memcpy(msg, strconcat, MUSE_MSG_MAX_LENGTH);

		MUSE_FREE(strconcat);
	}

	msg[client->cache_len + msg_len] = '\0';
...
int muse_core_ipc_recv_msg(int sock_fd, char *msg)
{
	...
	*(g_muse_core_ipc->key) = sock_fd;
	...
	int recv_len = MUSE_MSG_MAX_LENGTH - client->cache_len;

	if ((ret = recv(sock_fd, msg, recv_len, 0)) < 0) {
		strerror_r(errno, err_msg, MAX_ERROR_MSG_LEN);
		LOGE("fail to receive msg (%s)", err_msg);
	} else if (ret > 0) {
		msg[ret] = '\0';
		_muse_core_ipc_msg_concatenate(client, msg, recv_len);
...
int muse_core_ipc_recv_msg_ext(muse_client_h client, char *msg)
{
	...
	if ((ret = recv(client->fd, msg, recv_len, 0)) < 0) {
		strerror_r(errno, err_msg, MAX_ERROR_MSG_LEN);
		LOGE("fail to receive msg (%s)", err_msg);
	} else if (ret > 0) {
		msg[ret] = '\0';
