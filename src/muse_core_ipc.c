...
static void _muse_core_ipc_msg_concatenate(muse_client_h *client, char *msg, int msg_len)
{
	...
	int cache_len = (int)strlen((*client)->cache);
	if (cache_len > 0) {
		memcpy((*client)->cache + cache_len, msg, msg_len);
		(*client)->cache[cache_len + msg_len] = '\0';
		cache_len = (int)strlen((*client)->cache);
		memcpy(msg, (*client)->cache, MUSE_MSG_MAX_LENGTH);
		if (cache_len <= MUSE_MSG_MAX_LENGTH) {
			msg[cache_len] = '\0';
			memset((*client)->cache, 0, MUSE_MSG_MAX_LENGTH * 2);
		} else {
			char tmp[MUSE_MSG_MAX_LENGTH];
			int tmp_len;
			memset(tmp, 0, MUSE_MSG_MAX_LENGTH);
			memcpy(tmp, (*client)->cache + MUSE_MSG_MAX_LENGTH + 1, cache_len - MUSE_MSG_MAX_LENGTH + 1);
			tmp_len = (int)strlen(tmp);
			memset((*client)->cache, 0, MUSE_MSG_MAX_LENGTH * 2);
			memcpy((*client)->cache, tmp, tmp_len);
			(*client)->cache[tmp_len] = '\0';
		}
	}
}
...
static bool _muse_core_ipc_msg_complete_confirm(muse_client_h *client, char *msg, int len)
{
	...
	if (ptr_len > 0) {
		...
		(*client)->cache[ptr_len] = '\0';
		msg[idx + 1] = '\0';
		LOGD("cache %d %s", strlen((*client)->cache), (*client)->cache);
		return FALSE;
	}
...
int muse_core_ipc_recv_msg(int sock_fd, char *msg)
{
	...
	if ((ret = recv(sock_fd, msg, MUSE_MSG_MAX_LENGTH, 0)) < 0) {
		...
		*(g_muse_core_ipc->key) = sock_fd;
		...
		_muse_core_ipc_msg_concatenate(&client, msg, ret);
