				...
				if (muse_core_msg_json_deserialize(MUSE_API, module->recvMsg + module->msg_offset, &parse_len, &cmd, &err, MUSE_TYPE_INT)) {
					...
					case API_CREATE:
						if (muse_core_msg_json_deserialize(MUSE_MODULE, module->recvMsg + module->msg_offset, &parse_len, &api_module, &err, MUSE_TYPE_INT)) {
							...
							muse_core_module_get_instance()->set_module_state(module, MUSE_MODULE_STATE_READY);
							LOGD("module state: %d", muse_core_module_get_instance()->get_module_state(module));
					...
					case API_DESTROY:
						...
						break;
					default:
						muse_core_module_get_instance()->set_module_state(module, MUSE_MODULE_STATE_NULL);
