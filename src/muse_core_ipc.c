...
typedef struct MUSE_RecvDataHead {
	int marker;
	int id;
	int size;
} MUSE_RecvDataHead_t;

typedef struct MUSE_RecvData {
...
static MUSE_RecvData_t *_muse_core_ipc_new_qdata(char **recvBuff, int recvSize, int *allocSize);
		...
		if (recvLen <= 0) {
			...
			if (module) {
				...
				if (muse_core_msg_json_deserialize(MUSE_MODULE_ADDR,
					...
					module = (muse_module_t) module_addr;
					if (module)
...
{
	int qDataSize;
	MUSE_RecvData_t *qData = (MUSE_RecvData_t *)*recvBuff;
...
void muse_core_ipc_delete_data(char *data)
{
	...
	qData = (MUSE_RecvData_t *)(data - sizeof(MUSE_RecvDataHead_t));
