				...
				if (muse_core_msg_json_deserialize(MUSE_API, module->recvMsg + module->msg_offset, &parse_len, &cmd, &err, MUSE_TYPE_INT)) {
					...
					case API_CREATE:
						if (muse_core_msg_json_deserialize(MUSE_MODULE, module->recvMsg + module->msg_offset, &parse_len, &api_module, &err, MUSE_TYPE_INT)) {
							...
							LOGE("[%d] last module state: %d", muse_core_module_get_instance()->get_module_state(module->api_module));
							if (module->api_module == MUSE_CAMERA || module->api_module == MUSE_RECORDER)
								muse_core_module_get_instance()->set_flash_state(module->api_module, 1);
