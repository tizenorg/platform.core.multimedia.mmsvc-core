	...
	while (1) {
		...
		if (len <= 0) {
			...
			while (module->msg_offset < len) {
				if (muse_core_msg_json_deserialize("api", module->recvMsg + module->msg_offset, &parse_len, &cmd, &err, MUSE_TYPE_INT)) {
					switch (cmd) {
					case API_CREATE:
						if (muse_core_msg_json_deserialize("client", module->recvMsg + module->msg_offset, &parse_len, &disp_api, &err, MUSE_TYPE_INT)) {
		...
		if (recvLen <= 0) {
			...
			if (module) {
				...
				while ((qData = _muse_core_ipc_new_qdata(&recvBuff, currLen, &allocSize))
						!= NULL) {
					int qDataSize = qData->header.size + sizeof(MUSE_RecvDataHead_t);
				...
				if (muse_core_msg_json_deserialize("client_addr",
							recvBuff, NULL, &client_addr, NULL, MUSE_TYPE_POINTER)) {
					module = (muse_module_t) client_addr;
					if (module)
...
static RecvData_t *_muse_core_ipc_new_qdata(char **recvBuff, int recvSize, int *allocSize)
{
	...
	qDataSize = qData->header.size + sizeof(MUSE_RecvDataHead_t);
...
int muse_core_ipc_get_client_from_job(muse_core_workqueue_job_t *job)
{
	...
	module = (muse_module_t) job->user_data;
...
int muse_core_ipc_push_data(int sock_fd, const char *data, int size, int data_id)
{
	...
	if ((ret = send(sock_fd, &header, sizeof(MUSE_RecvDataHead_t), 0)) < 0)
...
void mue_core_ipc_delete_data(char *data)
{
	RecvData_t *qData;
	g_return_if_fail(data);

	qData = (RecvData_t *)(data - sizeof(MUSE_RecvDataHead_t));
...
char *muse_core_ipc_get_data(muse_module_t module)
{
	...
	if (qData) {
		rawData = (char *)qData + sizeof(MUSE_RecvDataHead_t);
		return rawData;
	}
