...
static void _muse_core_log_sig_abort(int signo)
{
	...
	if (g_muse_core_log) {
		...
		snprintf(client_buf, sizeof(client_buf) - 1, "[client name] %s", muse_core_config_get_instance()->get_host(muse_core_module_get_instance()->api_module));
		...
		memset(client_buf, '\0', sizeof(client_buf));
		snprintf(client_buf, sizeof(client_buf) - 1, "[client pid] %lu", (unsigned long) getpid());
		if (write(g_muse_core_log->log_fd, client_buf, strlen(client_buf)) != (int)strlen(client_buf))
		...
		memset(client_buf, '\0', sizeof(client_buf));
		snprintf(client_buf, sizeof(client_buf) - 1, "[client's latest called api] %s", _muse_core_log_get_msg());
		if (write(g_muse_core_log->log_fd, client_buf, strlen(client_buf)) != (int)strlen(client_buf))
