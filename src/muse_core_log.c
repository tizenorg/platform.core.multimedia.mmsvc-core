...
static void _muse_core_log_create_fd(void)
{
	...
	for (index = 0; index < MAX_FILE_NUM; index++) {
		...
		if (access(file[index], F_OK ) == 0) { /* if 0, then there is file */
			...
			LOGD("[%d] blksize: %d", g_muse_core_log->size, st.st_blksize);
	...
	g_muse_core_log->filename = g_strdup(file[selected_index]);
	/* open log file again */
	g_muse_core_log->log_fd = _muse_core_log_open_work(g_muse_core_log->filename);
