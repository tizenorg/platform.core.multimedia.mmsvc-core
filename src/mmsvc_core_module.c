...
void mmsvc_core_module_dll_symbol_dispatch(int cmd, Module module)
{
	...
	LOGD("cmd: %d\t client->module: %p", cmd, module->ch[MUSED_CHANNEL_MSG].module);
	g_module_symbol(module->ch[MUSED_CHANNEL_MSG].module, DISPATCHER, (gpointer *)&dispatcher);
...
gboolean mmsvc_core_module_close(Module module)
{
	...
	mmsvc_core_log_get_instance()->set_module_value(module->disp_api, module->ch[MUSED_CHANNEL_MSG].module, false);

	LOGD("Closing module %s", g_module_name(module->ch[MUSED_CHANNEL_MSG].module));
	if (!g_module_close(module->ch[MUSED_CHANNEL_MSG].module)) {
		LOGE("Couldn't close module %s: %s", g_module_name(module->ch[MUSED_CHANNEL_MSG].module), g_module_error());
		return FALSE;
	}
